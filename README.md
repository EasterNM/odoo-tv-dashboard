...
2. ดูตาราง SO จัดกลุ่มตามขนส่ง: ลูกค้า / จังหวัด / ขนส่ง / บิล / ชิ้น / แพ็ค / หมายเหตุ
3. ติ้ก SO ที่ขึ้นรถแล้ว (หรือ "เลือกทั้งหมด")
4. กด **"ยืนยันขึ้นรถ"** → กรอก ทะเบียนรถ / คนขับ / เวลาออกรถ
5. ระบบ:
   - สร้างเอกสาร **Dispatch Transfer** (เลขที่ `DT2026/XXXX`) ใน Odoo
   - link SO ทั้งหมดผ่าน many2many
   - mark `ขึ้นรถจัดส่งแล้ว = True` บนแต่ละ SO
   - สร้าง **PDF ใบสรุปขึ้นรถ** (Landscape A4, Thai font)
   - แนบ PDF เข้า chatter ของ Dispatch record
   - post chatter บนแต่ละ SO ระบุเลขเอกสาร
6. **PDF download อัตโนมัติ** หลัง confirm — พร้อมช่องเซ็นชื่อคนขับ / ผู้รับของ
...
### x_tv_dashboard_dispatc — Dispatch Transfer

Custom model สำหรับบันทึกแต่ละรอบการขึ้นรถ (1 record = 1 รอบ)

| Field Name | Type | ความหมาย |
|------------|------|-----------|
| `x_name` | char | เลขที่เอกสาร เช่น `DT2026/0001` |
| `x_route` | char | เส้นทางจัดส่ง |
| `x_plate` | char | ทะเบียนรถ |
| `x_driver` | char | คนขับ |
| `x_depart_time` | datetime | วันเวลาออกรถ (UTC) |
| `x_state` | selection | `draft` / `confirmed` |
| `x_so_ids` | many2many → sale.order | รายการ SO ทั้งหมดในรอบ |
...
| PDF generation | `fpdf2` + Sarabun Thai font |
...
│       ├── dispatch_service.py          ← ขึ้นรถ + Dispatch Transfer
...
### Models ที่ต้องสร้างใน Odoo Studio

**1. TV_dashboard_invoice_tranfer** (`x_tv_dashboard_invoice`)
- เปิด **Has Mail Thread** ✅
- Fields: `x_name`, `x_signer_name`, `x_signature`, `x_transfer_datetime`, `x_state`
- Line model (`x_tv_dashboard_invoice_line_1992d`): `x_so_id`

**2. TV_dashboard_dispatch** (`x_tv_dashboard_dispatc`)
- เปิด **Has Mail Thread** ✅
- Fields: `x_name`, `x_route`, `x_plate`, `x_driver`, `x_depart_time` (Datetime), `x_state`
- Many2many: `x_so_ids` → `sale.order`
